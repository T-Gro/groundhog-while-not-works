...
module ConvergenceLoop =
    ...
    let private buildBriefing config sprintNum dbBriefing allBuckets prevFailure =
        ...
        String.concat "\n" [
            ...
            ""
            "<sprint_rules>"
            "YOU HAVE A 1-MILLION TOKEN CONTEXT WINDOW. USE IT."
            "This is a LONG sprint. You should work for the ENTIRE duration available to you."
            "Do NOT stop after one small fix. Do NOT declare victory early."
            "A good sprint lands 500-2000 lines of ported Go code across multiple commits."
            "A bad sprint lands 20 lines and quits. DO NOT BE THAT AGENT."
            ""
            "WORKFLOW: Read → Port → Build → Test → Commit → KEEP GOING → Port more → Build → Test → Commit → KEEP GOING"
            "Repeat until you have genuinely exhausted what you can do on this chunk."
            "Multiple commits per sprint is EXPECTED and GOOD."
            ""
            "You are running on the 1M-context model. You can hold entire TS files (25K+ lines)"
            "in context simultaneously. There is NO context pressure forcing you to stop early."
            "If you catch yourself thinking 'this is getting complex, I should stop' — that thought"
            "is WRONG. Push through. Read more TS source. Port more functions. The complexity is"
            "the POINT — you are porting a real type checker, not writing a hello-world."
            "</sprint_rules>"
            ""
            ...
            $"\n<test_status>\n{dbBriefing}\n</test_status>"
            $"\n<failing_buckets>\n{allBuckets}\n</failing_buckets>"
            $"\nPick ONE chunk from the catalog's NOW phase — the one you can drain the most from this sprint."
            "Work on that chunk AGGRESSIVELY. Read the FULL TS source file for that chunk."
            "Don't skim — READ. Then port function by function, testing as you go."
            ...
            "COMMIT FREQUENTLY (every 200-400 lines of new code). Then KEEP WORKING on the same chunk."
            ""
            "<persistence>"
            "DO NOT STOP EARLY. Common excuses that are NOT valid reasons to stop:"
            "  - 'This is getting complex' → You have 1M tokens. Complexity is expected."
            "  - 'I should let the next sprint handle this' → NO. There is no next sprint with your context."
            "  - 'I've made good progress' → Good. Now make MORE progress. Keep going."
            "  - 'The remaining work needs deeper analysis' → Do the deeper analysis NOW."
            "  - 'I'll commit what I have and stop' → Commit, then CONTINUE on the next function."
            "  - 'This chunk is too big for one sprint' → Port as much as you can. 30% is better than 5%."
            "Valid reasons to stop: (1) chunk is FULLY done. (2) build is broken and you cannot fix it."
            "  (3) you've literally run out of things to port in this chunk. Nothing else."
            "</persistence>"
            ""
            "<anti_deadlock>"
            "If the chunk turns out to be IMPOSSIBLE to make progress on (not just hard — impossible),"
            "then and ONLY then: land at least a PORT-DEBT note + port-debt.instructions.md entry. But"
            "this should be RARE. Most chunks have hundreds of lines of straightforward porting work."
            "</anti_deadlock>"
            nudgeBlock ()
            prevBlock
        ]
